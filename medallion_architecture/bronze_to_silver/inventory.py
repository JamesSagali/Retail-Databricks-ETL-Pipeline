...
@dp.table(
    name="retail_b.retail_silver.inventory",
...
@dp.expect("valid stock quantity", "stock_quantity > 0")
def inventory_clean():
    #read source streaming table
    source_df = spark.readStream.table("retail_b.postgres_bronze.inventory")
    ...
    return(
        source_df.select(
            ...
            # Derived column for the inventory status
            F.when(
